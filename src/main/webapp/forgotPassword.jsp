...
.icon{width:72px;height:72px;border-radius:50%;background:rgba(79,142,247,.12);border:2px solid rgba(79,142,247,.2);display:flex;align-items:center;justify-content:center;font-size:1.8rem;color:var(--accent);margin:0 auto 24px;}
...
<div class="card">
  ...
  <h2 class="ct">Forgot Password?</h2>
  <p class="cs">Enter your registered email address. We'll verify your account and let you reset your password.</p>
  <%String err=(String)request.getAttribute("error");%>
  <%if(err!=null){%><div class="alert ae"><i class="fas fa-exclamation-circle"></i><%=err%></div><%}%>
  <form action="ForgotPasswordServlet" method="post">
    <input type="hidden" name="action" value="verify">
    <div class="fg"><label class="fl">Registered Email</label><div class="iw"><i class="fas fa-envelope ii"></i><input type="email" name="email" class="fi" placeholder="student@college.edu" required></div></div>
    <button type="submit" class="btn"><i class="fas fa-search"></i> Verify My Account</button>
  </form>
