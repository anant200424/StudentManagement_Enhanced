...
<title>Admin Reset Password — EduSphere</title>
...
body{font-family:'Satoshi',sans-serif;background:var(--navy);color:var(--text);min-height:100vh;display:flex;align-items:center;justify-content:center;padding:40px 20px;background-image:radial-gradient(ellipse 70% 60% at 20% 20%,#2a1a6c44,transparent);}
...
.btn{width:100%;padding:14px;border-radius:10px;border:none;background:linear-gradient(135deg,var(--purple),var(--accent));color:#fff;font-family:'Satoshi',sans-serif;font-size:.95rem;font-weight:700;cursor:pointer;transition:all .3s;display:flex;align-items:center;justify-content:center;gap:10px;margin-top:8px;}
...
<%
  ...
  if(sess==null||sess.getAttribute("resetEmail")==null){response.sendRedirect("adminForgotPassword.jsp");return;}
  String resetEmail=(String)sess.getAttribute("resetEmail");
%>
<div class="card">
  <div class="icon"><i class="fas fa-key"></i></div>
  <h2 class="ct">Set New Admin Password</h2>
  <p class="cs">Resetting for:<br><strong style="color:var(--purple);"><%=resetEmail%></strong></p>
  ...
  <form action="AdminForgotPasswordServlet" method="post">
    ...
    <div class="fg"><label class="fl">New Password</label><div class="iw"><i class="fas fa-lock ii"></i><input type="password" name="newPassword" class="fi" placeholder="New password" required minlength="6"></div></div>
    <div class="fg"><label class="fl">Confirm Password</label><div class="iw"><i class="fas fa-lock ii"></i><input type="password" name="confirmPassword" class="fi" placeholder="Repeat" required minlength="6"></div></div>
    <button type="submit" class="btn"><i class="fas fa-check"></i> Reset Admin Password</button>
  </form>
